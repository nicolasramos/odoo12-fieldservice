...
class Task(models.Model):
    ...
    consume_material_to_invoice = fields.Boolean(
        string="Re-invoice ?",
    )
            ...
            if 'stage_id' in vals or 'material_ids' in vals:
                if task.stage_id.consume_material:
                    ...
                    if todo_lines:
                        ...
                        if task.consume_material_to_invoice and task.sale_order_id:
                            todo_lines.create_sale_order_line()
    ...
    def create_sale_order_line(self):
        for line in self:
            ...
            line_id = self.env['sale.order.line'].create({
                ...
                'order_partner_id': order_id.partner_id.id,
                'product_id': line.stock_move_id.product_id.id,
                'product_uom_qty': line.stock_move_id.product_uom_qty,
                'product_uom': line.stock_move_id.product_uom.id,
                'price_unit': line.stock_move_id.product_id.lst_price,
                'customer_lead': 0,
                'company_id': order_id.company_id.id,
                'name': line.stock_move_id.product_id.name})
            line.stock_move_id.write({'sale_line_id': line_id.id})